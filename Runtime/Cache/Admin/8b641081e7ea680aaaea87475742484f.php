...
<script>
	...
	<div class="main-title cf">
		<h2>
			新增<?php echo (get_product_model($info["model_id"],'title')); ?> [
			...
			<?php if(isset($article)): ?>：<a href="<?php echo U('product/index','cate_id='.$info['category_id'].'&pid='.$article['id']);?>"><?php echo ($article["title"]); ?></a><?php endif; ?>
			]
		</h2>
	</div>
	<!-- 标签页导航 -->
            ...
            <?php if(is_array($fields[$key])): $i = 0; $__LIST__ = $fields[$key];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$field): $mod = ($i % 2 );++$i; if($field['is_show'] == 1 || $field['is_show'] == 2): ?><div class="form-item cf">
                    ...
                    <div class="controls">
                        <?php switch($field["type"]): case "num": ?><input type="text" class="text input-medium" name="<?php echo ($field["name"]); ?>" value="<?php echo ($field["value"]); ?>"><?php break;?>
                            ...
                            <?php case "linkage": echo get_linkage($field['extra'],$field['value']);?>
                                <script>
                                	$(function(){
										$(".<?php echo ($field['extra']); ?>").change(function(){
											var obj = $(this)
											$.post("<?php echo U('Linkage/make');?>",{id:$(this).val()},function(data){ obj.after(data); });
										});
									})
                                </script><?php break;?>
                            ...
                            <?php case "muchpic": ?><!--<link href="/Public/Admin/FontAwesome/css/font-awesome.css" rel="stylesheet" type="text/css" />-->
                                <div class="controls">
									<input type="file" id="upload_picture_<?php echo ($field["name"]); ?>">
									<input type="hidden" name="<?php echo ($field["name"]); ?>" id="cover_id_<?php echo ($field["name"]); ?>"/>
									<div class="upload-img-box">
								...
								function uploadPicture<?php echo ($field["name"]); ?>(file, data){
							    	...
							    	var src = '';
							        if(data.status){
										var oldid = $("#cover_id_<?php echo ($field["name"]); ?>").val();
										if(oldid==""){
											$("#cover_id_<?php echo ($field["name"]); ?>").val(data.id);
										}else{
											$("#cover_id_<?php echo ($field["name"]); ?>").val(data.id + "," + oldid);
										}
							        	src = data.url || '' + data.path
										var oldhtml = $("#cover_id_<?php echo ($field["name"]); ?>").parent().find('.upload-img-box').html();
							        	$("#cover_id_<?php echo ($field["name"]); ?>").parent().find('.upload-img-box').html(
							        		oldhtml + '<div class="upload-pre-item"><img src="' + src + '" onclick="tanchu(this);" /><!--<a href="javascript:;" onclick="delpic(this);" class="cbtn"></a><br><a href="javascript:;" class="icon-arrow-left aleft"></a><a href="javascript:;" class="icon-arrow-right aright"></a>--></div>'
							        	);
							        } else {
							        	updateAlert(data.info);
							        	setTimeout(function(){
