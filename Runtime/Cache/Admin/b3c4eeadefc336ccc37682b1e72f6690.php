                        ...
                        <?php switch($field["type"]): case "num": ?><input type="text" class="text input-medium" name="<?php echo ($field["name"]); ?>" value="<?php echo ($data[$field['name']]); ?>"><?php break;?>
                            ...
                            <?php case "muchpic": ?><!--<link href="/Public/Admin/FontAwesome/css/font-awesome.css" rel="stylesheet" type="text/css" />-->
                                <div class="controls">
									...
									<input type="hidden" name="<?php echo ($field["name"]); ?>" id="cover_id_<?php echo ($field["name"]); ?>" value="<?php echo ($data[$field['name']]); ?>"/>
									<div class="upload-img-box">
                                    <?php if($data[$field['name']] == null): else: ?>
                                    <?php $_result=parse_field($data[$field['name']]);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="upload-pre-item">
                                        	<img src="<?php echo (get_cover($vo,'path')); ?>" onclick="tanchu(this);" />
                                            <div class="close-btn" id="<?php echo ($vo); ?>" onclick="del(this);"></div>
								...
								function del(obj){
									var newid = $("#cover_id_picture").val()+',';
									var delid = obj.id+',';
									if(newid==delid){
										$("#cover_id_picture").val("");
										}else{
