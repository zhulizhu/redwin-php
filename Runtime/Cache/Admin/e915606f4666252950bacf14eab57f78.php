...
<body>
	...
	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$config): $mod = ($i % 2 );++$i;?><div class="form-item">
            ...
            <?php case "5": ?><input type="password" class="text input-large" name="config[<?php echo ($config["name"]); ?>]" value="<?php echo ($config["value"]); ?>"><?php break;?>
            <?php case "6": ?><label class="textarea">
                <textarea name="config[<?php echo ($config["name"]); ?>]"><?php echo ($config["value"]); ?></textarea>
                <?php echo hook('adminArticleEdit', array('name'=>"config[".$config['name']."]",'value'=>$config['value'],'height'=>'150px'));?>
