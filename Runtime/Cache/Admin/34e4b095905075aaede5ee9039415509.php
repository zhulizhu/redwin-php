...
<body>
 ...
 <h3>
 	...
 	<ul class="side-sub-menu <?php if(!in_array((ACTION_NAME), explode(',',"mydocument,draftbox,examine"))): ?>subnav-off<?php endif; ?>">
 		<li <?php if((ACTION_NAME) == "mydocument"): ?>class="current"<?php endif; ?>><a class="item" href="<?php echo U('product/myproduct');?>">我的产品</a></li>
 		<?php if(($show_draftbox) == "1"): ?><li <?php if((ACTION_NAME) == "draftbox"): ?>class="current"<?php endif; ?>><a class="item" href="<?php echo U('product/draftbox');?>">草稿箱</a></li><?php endif; ?>
		<li <?php if((ACTION_NAME) == "draftbox"): ?>class="examine"<?php endif; ?>><a class="item" href="<?php echo U('product/examine');?>">待审核</a></li>
 	</ul>
	...
	<?php if(($show_recycle) == "1"): ?><h3>
        <em class="recycle"></em>
        <a href="<?php echo U('product/recycle');?>">回收站</a>
    </h3><?php endif; ?>
</div>
<script>
    $(function(){
	...
	<div class="main-title">
		...
		<?php if(is_array($rightNav)): $i = 0; $__LIST__ = $rightNav;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$nav): $mod = ($i % 2 );++$i;?><a href="<?php echo U('Product/index','cate_id='.$nav['id']);?>"><?php echo ($nav["title"]); ?></a>
			<?php if(count($rightNav) > $i): ?><i class="ca"></i><?php endif; endforeach; endif; else: echo "" ;endif; ?>
		<?php if(isset($article)): ?>：<a href="<?php echo U('product/index','cate_id='.$cate_id.'&pid='.$article['id']);?>"><?php echo ($article["title"]); ?></a><?php endif; ?>
		]
	...
	<div class="cf">
		<div class="fl">
			<div class="btn-group">
				<?php if(($allow) > "0"): ?><button class="btn document_add" <?php if(count($model) == 1): ?>url="<?php echo U('product/add',array('cate_id'=>$cate_id,'pid'=>I('pid',0),'model_id'=>$model));?>"<?php endif; ?>>新 增
						<?php if(count($model) > 1): ?><i class="btn-arrowdown"></i><?php endif; ?>
					</button>
					<?php if(count($model) > 1): ?><ul class="dropdown nav-list">
						<?php if(is_array($model)): $i = 0; $__LIST__ = $model;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li><a href="<?php echo U('product/add',array('cate_id'=>$cate_id,'model_id'=>$vo,'pid'=>I('pid',0)));?>"><?php echo (get_product_model($vo,'title')); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
					</ul><?php endif; ?>
				<?php else: ?>
            ...
            <button class="btn ajax-post" target-form="ids" url="<?php echo U("Product/setStatus",array("status"=>1));?>">启 用</button>
			<button class="btn ajax-post" target-form="ids" url="<?php echo U("Product/setStatus",array("status"=>0));?>">禁 用</button>
			<!--<button class="btn ajax-post" target-form="ids" url="<?php echo U("Product/move");?>">移 动</button>-->
			<button class="btn ajax-post" target-form="ids" url="<?php echo U("Product/copy");?>">复 制</button>
			<button class="btn ajax-post" target-form="ids" hide-data="true" url="<?php echo U("Product/paste");?>">粘 贴</button>
			...
			<button class="btn ajax-post confirm" target-form="ids" url="<?php echo U("Product/setStatus",array("status"=>-1));?>">删 除</button>
		...
		<div class="search-form fr cf">
			<div class="sleft">
				...
				<a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('Product/index','pid='.I('pid',0).'&cate_id='.$cate_id,false);?>"><i class="btn-search"></i></a>
			</div>
