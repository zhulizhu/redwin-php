...
class PurchaseModel extends Model{
	 ...
	 * @return array              文档列表
	 */
	public function lists($category,$order = '`id` DESC', $status = 1, $field = true,$addon = false){
		$Child = D('Purcate')->getChildrenId($category);
		if($Child!=""){
			$map = $this->listMap($Child, $status);
		}else{
	 ...
	 * pos=2首页推荐
	 */
	public function positions($id,$pos){
		$map['status']=1;
		$map['type'] = 3;
		if($id!=0){
			$map['category_id'] =$id;
		...
		$data = M('purchase')->where($map)->Order('`id` DESC')->select();
		return $data;
	 ...
	 * @return array              文档列表
	 */
	public function Screening($category,$purchaseId,$order = '`id` DESC', $status = 1, $field = true,$addon = false){
		$Child = D('Purcate')->getChildrenId($category);
		if($Child!=""){
			$map = $this->listMap($Child, $status);
		}else{
	 ...
	 * 店铺查询
	 */
	public function ShopList($category, $order = '`id` DESC', $status = 1,$field = true,$PurchaseId=0){
		$map["uid"] =$category;
		if ($PurchaseId){
			$map["id"][]='in';
			$map["id"][]=$PurchaseId;
	...
	private function logic($model){
		return D(get_purchase_model($model, 'name'), 'Logic');
	}
