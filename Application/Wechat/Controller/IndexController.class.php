...
class IndexController extends HomeController {
	...
	public function index() {
		...
		$where['position'] = 3;
		$where['status'] = 1;
		$list = M("Product")->where($where)->limit(5)->order("update_time desc")->select();
		$this->assign("prolist",$list);

		$where = array();
		$where['category_id'] = 144;
		$where['status'] = 1;
		$did = M("Document")->where($where)->getField("id");
		$content = M("document_article")->where("id=".$did)->getField("content");
		$this->assign("content",$content);

		$this->display ();
	}

	public function server_time($end_time)
	{
		$s = $this->gettime(NOW_TIME,$end_time);
	...
	public function gettime($time_s,$time_n){
		...
		if($time >= 3600){
			$strtime[] = intval($time/3600);
			$time = $time % 3600;
		}else{
			$strtime[] = '0';
		}
		if($time >= 60){
			$strtime[] = intval($time/60);
			$time = $time % 60;
		}else{
			$strtime[] = '0';
		}
