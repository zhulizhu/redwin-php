...
class PurchaseController extends AdminController {
	...
	/* 保存允许访问的公共方法 */
	static protected $allow = array( 'draftbox','myPurchase');
    ...
    protected function checkDynamic(){
        ...
        switch(strtolower(ACTION_NAME)){
            ...
            case 'update':  //更新
                $doc_id  =  I('id');
                $cate_id =  M('Purchase')->where(array('id'=>$doc_id))->getField('category_id');
                break;
            case 'setstatus': //更改状态
            case 'permit':    //回收站
                $doc_id  =  (array)I('ids');
                $cate_id =  M('Purchase')->where(array('id'=>array('in',$doc_id)))->getField('category_id',true);
    ...
    protected function getMenu(){
        //获取动态分类
        $cate_auth  =   AuthGroupModel::getAuthPurcate(UID);	//获取当前用户所有的内容权限节点
        $cate_auth  =   $cate_auth == null ? array() : $cate_auth;
        $cate       =   M('Purcate')->where(array('status'=>1))->field('id,title,pid,allow_publish')->order('pid,sort')->select();
        ...
        //是否展开分类
        $hide_cate = false;
        if(ACTION_NAME != 'recycle' && ACTION_NAME != 'draftbox' && ACTION_NAME != 'myPurchase'){
            $hide_cate  =   true;
        }

        //生成每个分类的url
        foreach ($cate as $key=>&$value){
            $value['url']   =   'Purchase/index?cate_id='.$value['id'];
            ...
            if(!empty($value['_child'])){
                ...
                foreach ($value['_child'] as $ka=>&$va){
                    $va['url']      =   'Purchase/index?cate_id='.$va['id'];
                    if(!empty($va['_child'])){
                        foreach ($va['_child'] as $k=>&$v){
                            $v['url']   =   'Purchase/index?cate_id='.$v['id'];
                            $v['pid']   =   $va['id'];
        ...
        //获取面包屑信息
        $nav = get_parent_purcate($cate_id);
        $this->assign('rightNav',   $nav);

        //获取回收站权限
        $show_recycle = $this->checkRule('Admin/Purchase/recycle');
    ...
    public function index($cate_id = null){
        ...
        //获取模型信息
        $model = M('Model')->getByName('purchase');
        ...
        if(!empty($cate_id)){   //没有权限则不查询数据
            //获取分类绑定的模型
            $models         =   get_purcate($cate_id, 'model');
            $allow_reply    =   get_purcate($cate_id, 'reply');//分类文档允许回复
            $pid            =   I('pid');
            if ( $pid==0 ) {
            	...
            	$template = $this->indexOfArticle( $cate_id ); //转入默认文档列表方法
            	$result = M('purcate')->where(array('id'=>$cate_id))->find();
            	$result = $result['model_append'];
            	$this->assign('model',$result);
            	/*
                //开发者可根据父文档的模型类型,按需定制子文档列表
                $doc_model = M('Purchase')->where(array('id'=>$pid))->find();
                dump($doc_model);
                exit;
    ...
    protected function indexOfReply($cate_id) {
        ...
        $prefix   = C('DB_PREFIX');
        $l_table  = $prefix.('purchase');
        $r_table  = $prefix.('purchase_purlist');
        ...
        if($map['pid']){
            // 获取上级文档
            $article    =   $Purchase->field('id,title,type')->find($map['pid']);
            $this->assign('article',$article);
        }
        //检查该分类是否允许发布内容
        $allow_publish  =   get_purcate($cate_id, 'allow_publish');
    ...
    protected function indexOfArticle($cate_id){
        ...
        $list = $this->lists($Purchase,$map,'level DESC,id DESC');
        int_to_string($list);
        if($map['pid']){
            // 获取上级文档
            $article    =   $Purchase->field('id,title,type')->find($map['pid']);
     ...
     * @author huajie <banhuajie@163.com>
     */
    public function setStatus($model='Purchase'){
        return parent::setStatus('Purchase');
    }
    ...
    public function add(){
        ...
        //检查该分类是否允许发布
        $allow_publish = D('Purchase')->checkCategory($cate_id);
        !$allow_publish && $this->error('该分类不允许发布内容！');

        /* 获取要编辑的扩展模型模板 */
        $model      =   get_purchase_model($model_id);
        //处理结果
        ...
        if($info['pid']){
            // 获取上级文档
            $article            =   M('purchase')->field('id,title,type')->find($info['pid']);
            $this->assign('article',$article);
        }
    ...
    public function edit(){
        ...
        if($data['pid']){
            // 获取上级文档
            $article        =   M('Purchase')->field('id,title,type')->find($data['pid']);
            $this->assign('article',$article);
        }
        ...
        /* 获取要编辑的扩展模型模板 */
        $model      =   get_Purchase_model($data['model_id']);
        $this->assign('model',      $model);
    ...
    public function update(){
        $res = D('Purchase')->update(I('post.'));
        if(!$res){
            $this->error(D('Purchase')->getError());
        }else{
    ...
    public function examine(){
        ...
        $list = $this->lists(M('Purchase'),$map,'update_time desc');
        //处理列表数据
        if(is_array($list)){
            foreach ($list as $k=>&$v){
    ...
    public function autoSave(){
        ...
        if($res !== false){
            ...
            $this->error('保存草稿失败：'.D('Purchase')->getError());
        }
    }
     ...
     * @author huajie <banhuajie@163.com>
     */
    public function mypurchase($status = null, $title = null){
        //获取左边菜单
        $this->getMenu();
        ...
        //只查询pid为0的文章
        $map['pid'] = 0;
        $list = $this->lists($Purchase,$map,'update_time desc');
        int_to_string($list);
        // 记录当前列表页的cookie
        Cookie('__forward__',$_SERVER['REQUEST_URI']);
        $this->assign('status', $status);
    ...
    public function paste() {
        ...
        if(!empty($moveList)) {// 移动	TODO:检查name重复
            ...
            foreach ($copyList as $key=>$value){
                ...
                if($result){
                    $logic      =   D(get_Purchase_model($data['model_id'],'name'),'Logic');
    ...
    protected function checkPaste($list, $cate_id, $pid){
        ...
        foreach ($list as $key=>$value){
            ...
            // 移动文档的所属文档模型
            $modelType  =   $Purchase->getFieldById($value,'model_id');
        ...
        foreach ($list as $key=>$value){
            // 移动文档的所属文档模型
            $modelType  =   $Purchase->getFieldById($value,'type');
            if(!in_array($modelType,explode(',',$typeList))) {
                $return['status'] = 0;
                $return['info'] = '当前分类的文档类型不支持编号为 '.$value.' 的数据！';
                return $return;
            }
            $res = $Purchase->checkPurchaseType($modelType, $pid);
            if(!$res['status']){
                $return['status'] = 0;
    ...
    public function sort(){
        if(IS_GET){
            ...
            $list = M('Purchase')->where($map)->field('id,title')->order('level DESC,id DESC')->select();
            ...
            foreach ($ids as $key=>$value){
                $res = M('Purchase')->where(array('id'=>$value))->setField('level', $key+1);
            }
            if($res !== false){
                $this->success('排序成功！');
            }else{
    ...
    public function get_userfiled($model_id,$cate_id,$group = true){
    	...
    	if($group){
    		$sort = M('purcateModel')->where('pid='.$cate_id)->field('field_sort')->find();
