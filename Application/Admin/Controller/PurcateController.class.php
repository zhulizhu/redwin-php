 ...
 * 产品分类管理控制器
 */
class PurcateController extends AdminController {
    /**
     * 分类管理列表
     * @author 麦当苗儿 <zuojiazi@vip.qq.com>
     */
    public function index(){
        $tree = D('purcate')->getTree(0,'id,name,title,sort,pid,allow_publish,status');
    ...
    public function edit($id = null, $pid = 0){
        ...
        if(IS_POST){ //提交表单
            ...
            //获取该分类的模型
            $model = M('PurcateModel')->field(true)->where('pid='.$id)->find();
        ...
        if(IS_POST){ //提交表单
            ...
            if($rel){
            	...
            	if(empty($_POST['pid'])){//顶级分类继承文档模型
            		...
            		$model = M('PurcateModel')->where('pid='.$_POST['pid'])->find();
            	}
            	$model['id'] = "";
            	$model['pid'] = $rel;
            	if(M('PurcateModel')->add($model)){
    ...
    public function remove(){
        ...
        $child = M('Purcate')->where(array('pid'=>$cate_id))->field('id')->select();
        if(!empty($child)){
