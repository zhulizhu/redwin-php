 ...
 * 产品文档基础模型
 */
class PurcateModelModel extends Model{
     ...
     * @author huajie <banhuajie@163.com>
     */
    public function update($theArray = null){
        /* 获取数据对象 */
    	if($theArray != null){
    		$data = $this->create($theArray);
    	}else{
    		$data = $this->create();
    	}
        if(empty($data)){
		...
		//记录行为
		//action_log('update_usermodel','usermodel',$data['id'] ? $data['id'] : $id,UID);
