...
class AuthGroupModel extends Model {
    ...
    const AUTH_EXTEND_CATEGORY_TYPE = 1;// 新闻分类权限标识
    const AUTH_EXTEND_MODEL_TYPE    = 2;// 模型分类权限标识
    const AUTH_EXTEND_PROCATE_TYPE  = 3;// 产品分类权限标识
     ...
     *
     */
    static public function getAuthProcate($uid){
    	return self::getAuthExtend($uid,self::AUTH_EXTEND_PROCATE_TYPE,'AUTH_CATEGORY');
    }

/**
     * 返回用户拥有管理权限的团购产品分类id列表
     *
     * @param int     $uid  用户id
     * @return array
     *
     ...
     *
     */
    static public function getAuthPurcate($uid){
    	return self::getAuthExtend($uid,self::AUTH_EXTEND_PROCATE_TYPE,'AUTH_CATEGORY');
    }
     ...
     *
     */
    static public function addToProcate($gid,$cid){
    	return self::addToExtend($gid,$cid,self::AUTH_EXTEND_PROCATE_TYPE);
    }
     ...
     * @param array|string $cid  栏目分类id列表
     */
    public function checkProcateId($cid){
    	return $this->checkId('Procate',$cid, '以下分类id不存在:');
    }
