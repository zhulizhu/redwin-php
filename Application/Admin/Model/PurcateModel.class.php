 ...
 */
class PurcateModel extends Model{
    ...
    protected $_auto = array(
    	...
    	array('model_append', 'arr2str', self::MODEL_BOTH, 'function'),
    	array('model_append', null, self::MODEL_BOTH, 'ignore'),
    ...
    public function getTopId($cate){
    	$Document = D('Document');
    	$theArray = $Document->detail($cate);
    	$pid = $theArray["category_id"];
    	$temp = array();
    	while ( $pid != 0){
    		$temp = $this->info($pid);
    		$pid = $temp['pid'];
    ...
    protected function _after_find(&$data, $options){
        ...
        /* 分割附加模型 */
        if(!empty($data['model_append'])){
        	$data['model_append'] = explode(',', $data['model_append']);
        }
