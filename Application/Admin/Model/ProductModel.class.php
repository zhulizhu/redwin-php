...
class ProductModel extends Model{
     ...
     * @author huajie <banhuajie@163.com>
     */
    public function autoAdd($data = null){
    	...
    	if(empty($data['id'])){ //新增数据
			...
			<params>
			<param><value><string>".$data["title"]."</string></value></param>
			<param><value><string>http://".$_SERVER['HTTP_HOST']."</string></value></param>
			<param><value><string>".U('Home/Article/detail@'.$_SERVER['HTTP_HOST'].'?id='.$id)."</string></value></param>
			<param><value><string>http://".$_SERVER['HTTP_HOST']."</string></value></param>
			</params>
			</methodCall>";;
    	...
    	$logdata['id'] = $id;
    	$logdata['content'] = $data['title'];
    	$logdata['parse'] = 0;
    	$logdata['template'] = "";
    	$logdata['bookmark'] = 0;
    	M('DocumentArticle')->add($logdata);
    ...
    public function checkCategory($id){
        $publish = get_procate($id, 'allow_publish');
     ...
     * @author huajie <banhuajie@163.com>
     */
    public function checkProductType($type = null, $pid = null){
    	$res = array('status'=>1, 'info'=>'');
		if(empty($type)){
			return array('status'=>0, 'info'=>'文档类型不能为空');
		}
