...
class CategoryController extends ManagerController {
    ...
    public function edit($id = null, $pid = 0){
        ...
        if(IS_POST){ //提交表单
            ...
            //获取该分类的模型
            $model = M('CategoryModel')->field(true)->where('pid='.$id)->find();
    		$map = array('model_id'=> array("in", array($info['model'][0],'1')));
        ...
        if(IS_POST){ //提交表单
            ...
            if($rel){
            	...
            	if(empty($_POST['pid'])){//顶级分类继承文档模型
            		...
            		$model = M('CategoryModel')->where('pid='.$_POST['pid'])->find();
            	}
            	$model['id'] = "";
            	$model['pid'] = $rel;
            	if(M('CategoryModel')->add($model)){
