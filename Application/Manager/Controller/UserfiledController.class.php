 ...
 * @author huajie <banhuajie@163.com>
 */
class UserfiledController extends ManagerController {
    ...
    public function index(){
    	...
    	$model = M('Usermodel')->where('id='.$model_id)->find();
    	
    	
    	$list = $this->lists('Userfiled', $map);
    	int_to_string($list);
    ...
    public function add(){
        $model_id   =   I('get.model_id');
        $model      =   M('Usermodel')->field('title,name,field_group')->find($model_id);
        $this->assign('model',$model);
    ...
    public function edit(){
        ...
        $model  =   M('Usermodel')->field('title,name,field_group')->find($data['model_id']);
        $this->assign('model',$model);
    ...
    public function update(){
        $res = D('Userfiled')->update();
        if(!$res){
            $this->error(D('Userfiled')->getError());
        }else{
