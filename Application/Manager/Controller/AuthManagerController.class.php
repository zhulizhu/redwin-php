 ...
 * @author 朱亚杰 <zhuyajie@topthink.net>
 */
class AuthManagerController extends ManagerController{
    ...
    public function addToProcate(){
    	...
    	if( $cid && !$AuthGroup->checkProcateId($cid)){
    		$this->error($AuthGroup->error);
    	}
    	if ( $AuthGroup->addToProcate($gid,$cid) ){
    		$this->success('操作成功');
    	}else{
    		$this->error('操作失败');
    	}
