 ...
 * @author huajie <banhuajie@163.com>
 */
class UsermodelController extends ManagerController {
    ...
    public function edit(){
    	...
    	$fields = M('Userfiled')->where(array('model_id'=>$data['id']))->field('id,name,title,is_show')->select();
    	//是否继承了其他模型
    	if($data['extend'] != 0){
    		$extend_fields = M('Userfiled')->where(array('model_id'=>$data['extend']))->field('id,name,title,is_show')->select();
    ...
    public function update(){
        $res = D('Usermodel')->update();
        if(!$res){
            $this->error(D('Usermodel')->getError());
        }else{
