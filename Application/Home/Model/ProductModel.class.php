...
class ProductModel extends Model{
	 ...
	 * @return array              文档列表
	 */
	public function lists($category,$state=0,$order = '`id` DESC', $status = 1, $field = true,$addon = false){
		$Child = D('Procate')->getChildrenId($category);
		if($Child!=""){
			$map = $this->listMap($Child,$state, $status);
		}else{
		    $map = $this->listMap($category,$state, $status);
		}
		$result = $this->field($field)->where($map)->order($order)->select();
		/*文档表*/
		if($addon){
			$group_id = M("AuthGroupAccess")->where("uid=".HUID)->getField("group_id");//获取当前用户的分组ID
			for($i=0;$i<count($result);$i++){
				/* 获取模型数据 */
				$logic  = $this->logic($result[$i]['model_id']);
				$detail = $logic->detail($result[$i]['id']); //获取指定ID的数据
				if(!$detail){
					$this->error = $logic->getError();
					return false;
				}
				
				/*这里进行前台展示价格判断，并不是每个项目都有*/
				if($group_id==5){//经销商价格
					$result[$i]['price'] = $detail['jxprice'];
				}
				if($group_id==7){//供应商价格
					$result[$i]['price'] = $detail['gyprice'];
				}
				$result[$i]['addon'] = $detail;
			}
		}
		return $result;
	}
	 ...
	 * 店铺查询
	 */
	public function ShopList($category, $order = '`id` DESC', $status = 1,$field = true,$ProductId=0){
		$map["uid"] =$category;
		if ($ProductId){
			$map["id"][]='in';
			$map["id"][]=$ProductId;
	 ...
	 * @return array             数据列表
	 */
	public function position($pos,$category = null,$state,$limit = null, $field = true){
		$map = $this->listMap($category,$state,1, $pos);
	...
	private function logic($model){
		return D(get_product_model($model, 'name'), 'Logic');
	}
	 ...
	 * @return array             查询条件
	 */
	private function listMap($category, $state,$status = 1, $pos = null){
		/* 设置状态 */
		$map = array('status' => $status, 'pid' => 0,'state' => $state);
	 ...
	 * 模糊查询
	 */
	public function Fuzzy($where = array(), $order = '`id` DESC', $status = 1, $field = true,$addon = false  ){
		...
		$result = $this->field($field)->where($map)->order($order)->select();
		if($result==null){
			$result = $this->field($field)->where("status=1")->order($order)->select();
		}
		/*文档表*/
		if($addon){
			for($i=0;$i<count($result);$i++){
	 ...
	 * 字段查询
	 */
	public function Fuzzys($where = array(), $order = '`id` DESC', $status = 1, $field = true,$addon = false  ){
		foreach ($where as $key => $value){
			$map[$key] = $value;
		}
		$map['status'] = $status;
