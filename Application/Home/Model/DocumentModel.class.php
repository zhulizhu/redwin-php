...
class DocumentModel extends Model{
	 ...
	 * @return array              文档列表
	 */
	public function lists($category,$order = '`id` DESC', $status = 1, $field = true,$addon = false,$data){
	 ...
	 * 模糊查询
	 */
	public function Fuzzy($category, $order = '`id` DESC', $status = 1, $field = true,$addon = false , $child,$key){
		if($child){
			$category = D('Category')->getChildrenId($category);
		}
		$map = $this->listMap($category, $status);
