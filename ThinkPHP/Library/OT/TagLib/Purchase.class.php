...
class Purchase extends TagLib{
	...
	public function _list($tag, $content){
		...
		$parse  = '<?php ';
		if($child!="false"){
			$parse .= '$category_id=D(\'Purcate\')->getChildrenId('.$cate.');';
		}else{
			$parse .= '$category_id='.$cate.';';
		}
		$parse .= '$__LIST__ = D(\'Purchase\')->page(!empty($_GET["page"])?$_GET["page"]:1,'.$row.')->lists(';
		$parse .= '$category_id, \'`level` DESC,`id` DESC\', 1,';
		$parse .= $field . ','.$addon.');';
		$parse .= ' ?>';
	...
	public function _position($tag, $content){
		...
		$name   = $tag['name'];
		$parse  = '<?php ';
		$parse .= '$__POSLIST__ = D(\'Purchase\')->position(';
		$parse .= $pos . ',';
		$parse .= $cate . ',';
		$parse .= $limit . ',';
		$parse .= $field . ');';
